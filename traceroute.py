    ...
    for ttl in range(1, MAX_HOPS):
        for tries in range(TRIES):
            ...
            try:
                ...
                types = icmpv4_pkt_type(recvPacket)
                # Fill in end
                ...
                if types == 11:
                    tot_bytes = struct.calcsize("d")
                    timeSent = struct.unpack("d", recvPacket[28:28 + tot_bytes])[0]
                    # Fill in start
                    ...
                    df = df.affend({'Hop Count': str(ttl), 'Try': str(tries + 1), 'IP': '*', 'Hostname': '*', 'Response Code': 'Unknown'}, ignore_index=True)
                    # Fill in end
                break
        mySocket.close()
    return df
